    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        when (currentNightMode) {
            ...
            Configuration.UI_MODE_NIGHT_YES -> {
                ...
                txtSex.setTextColor(ContextCompat.getColor(this, R.color.black))
                txtApe.setTextColor(ContextCompat.getColor(this, R.color.black))
                btnCambiarContrasena.setTextColor(ContextCompat.getColor(this, R.color.white))
                btnCambiarContrasena.setBackgroundColor(ContextCompat.getColor(this, R.color.Turquesa2))
            }
        }
        ...
        btnCambiarContrasena.setOnClickListener {
            CoroutineScope(Dispatchers.Main).launch {
                ...
                val codigoRecu = (1000..9999).random()
                withContext(Dispatchers.IO) {
                    EmailSender().enviarCorreo(
                        "$userEmail",
                        "Cambio de contraseña Delta",
                        "$codigoRecu"
                    )
                }

                // Mostrar el segundo dialog para el código de verificación
                ...
                // Navegación entre los campos de código
                txtRecu1.addTextChangedListener { if (txtRecu1.text.isNotEmpty()) txtRecu2.requestFocus() }
                txtRecu2.addTextChangedListener { txtRecu3.requestFocus() }
                txtRecu3.addTextChangedListener { txtRecu4.requestFocus() }

                txtRecu4.setOnKeyListener { _, keyCode, event ->
                ...
                txtRecu3.setOnKeyListener { _, keyCode, event ->
                    if (keyCode == KeyEvent.KEYCODE_DEL && event.action == KeyEvent.ACTION_DOWN) {
                        txtRecu3.setText("")
                ...
                txtRecu2.setOnKeyListener { _, keyCode, event ->
                    if (keyCode == KeyEvent.KEYCODE_DEL && event.action == KeyEvent.ACTION_DOWN) {
                        txtRecu2.setText("")
                ...
                btnRecuSig.setOnClickListener {
                    ...
                    if (intentoRecu == codigoRecu) {
                        Log.d("Recuperacion", "Código correcto")
                        dialog2.dismiss()

                        // Mostrar el tercer dialog para cambiar la contraseña
                ...
                if (enteredPassword.isNotEmpty() && enteredPassword == repeatedPassword) {
                    if (withContext(Dispatchers.IO) {
                        ...
                        if (isDeleted) {
                            ...
                            val intent =
                                Intent(this@activity_cuenta_confi, splash_screen::class.java)
                            startActivity(intent)
                            finish()
                            finish()
                        } else {
                            Toast.makeText(
                                this@activity_cuenta_confi,
                                "No se encontró el usuario con email $userEmail",
                                Toast.LENGTH_SHORT
                            ).show()
                        }
                        dialog.dismiss()
    ...
    suspend fun deleteUser(emailUsuario: String): Boolean {
        return withContext(Dispatchers.IO) {
            ...
            try {
                ...
                if (objConexion != null) {
                    val statement =
                        objConexion.prepareStatement("DELETE FROM tbUsuarios WHERE emailUsuario = ?")
                    statement.setString(1, emailUsuario)
